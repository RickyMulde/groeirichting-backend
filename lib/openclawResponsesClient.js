...
async function postResponses({ conversationId, input, agentId, tools }) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      if (!loggedBodyOnce) {
        // Mask base64 data in input_file items voor logging.
        const safeBody = {
          ...body,
          input: Array.isArray(body.input)
            ? body.input.map((item) => {
                if (item && item.type === 'input_file' && item.source && typeof item.source.data === 'string') {
                  return {
                    ...item,
                    source: {
                      ...item.source,
                      data: `<base64, ${item.source.data.length} chars>`
                    }
                  }
                }
                return item
        ...
        console.log('GroeiCockpit [OPENCLAW_REQUEST_BODY]', JSON.stringify(safeBody))
        loggedBodyOnce = true
      }
